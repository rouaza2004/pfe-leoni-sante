...
export default function IncidentsAvecBonPage() {
  ...
  const loadItems = async () => {
    try {
      setLoading(true);
      setErr("");
      const res = await api.get("/medical/incidents-avec-bon/");
      setItems(Array.isArray(res.data) ? res.data : []);
    } catch (e) {
      console.error(e);
      setErr("Erreur chargement incidents avec bon.");
    } finally {
      setLoading(false);
    }
  };
  ...
  const stats = useMemo(() => {
    ...
    const thisMonth = items.filter((item) => {
      const d = item.date_incident ? new Date(item.date_incident) : null;
    ...
    const causeCounts = countBy("cause");
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setErr("");
      if (editing) {
        await api.patch(`/medical/incidents-avec-bon/${editing}/`, form);
      } else {
        await api.post("/medical/incidents-avec-bon/", form);
      }
      setShowForm(false);
      setEditing(null);
      setForm(emptyForm);
  ...
  const handleEdit = (item) => {
    setEditing(item.id);
    setForm({
      date_bon: item.date_bon || "",
      matricule: item.matricule || "",
      nom_prenom: item.nom_prenom || "",
      telephone: item.telephone || "",
      numero_assurance: item.numero_assurance || "",
  ...
  const handleDelete = async (item) => {
    ...
    try {
      setErr("");
      await api.delete(`/medical/incidents-avec-bon/${item.id}/`);
      await loadItems();
    } catch (e) {
      console.error(e);
    ...
    <div className="space-y-6">
      <div className="rounded-3xl bg-white p-6 shadow-sm ring-1 ring-slate-200">
        <div className="flex flex-wrap items-center justify-between gap-4">
          <div>
            <h1 className="text-2xl font-bold text-slate-900">Incidents + Bon</h1>
            <p className="text-sm text-slate-500">Suivi des incidents avec bon</p>
          </div>
          <button
            onClick={() => {
              setEditing(null);
      ...
      <div className="rounded-3xl bg-white p-5 shadow-sm ring-1 ring-slate-200">
        <div className="mb-4">
          ...
          <p className="text-sm text-slate-500">Incidents avec bon</p>
        </div>
              ...
              <table className="min-w-full text-sm">
                <thead>
                  <tr className="border-b border-slate-200 text-left text-slate-500">
                    <th className="px-3 py-3 font-medium">Date Bon</th>
                    ...
                    <th className="px-3 py-3 font-medium">N° Assurance</th>
                    <th className="px-3 py-3 font-medium">Date Incident</th>
                  ...
                  {pageItems.length > 0 ? (
                    pageItems.map((item) => (
                      <tr key={item.id} className="border-b border-slate-100 last:border-0">
                        <td className="px-3 py-4">{item.date_bon || "--"}</td>
                        ...
                        <td className="px-3 py-4">{item.numero_assurance || "--"}</td>
                        <td className="px-3 py-4">{item.date_incident || "--"}</td>
                        <td className="px-3 py-4">{item.destination || "--"}</td>
                        <td className="px-3 py-4">{item.infirmier || "--"}</td>
                        <td className="px-3 py-4">{item.cause || "--"}</td>
                        <td className="px-3 py-4">{item.lesion || "--"}</td>
      ...
      >
        <form
          id="incident-avec-bon-form"
          onSubmit={handleSubmit}
          className="grid gap-4 lg:grid-cols-2"
        >
          {[
            ["date_bon", "Date Bon", "date"],
            ["matricule", "Matricule"],
            ["nom_prenom", "Nom & Prénom"],
            ["telephone", "Téléphone"],
            ["numero_assurance", "N° Assurance"],
            ["date_incident", "Date Incident", "date"],
